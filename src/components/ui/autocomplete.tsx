...
const M3AutocompleteStyles = () => (
  <style dangerouslySetInnerHTML={{
    ...
    .m3-auto-container[data-virtualized="true"] .m3-auto-item {
...
export function Autocomplete({
  ...
  const shouldVirtualize = virtualize && lastFilteredItems.length > 40;
  const virtualItemHeight = 44;
  const virtualOverscan = 6;
  const [virtualScrollTop, setVirtualScrollTop] = React.useState(0);

  React.useEffect(() => {
    if (!shouldVirtualize || !scrollViewportRef.current) return;
  ...
  React.useEffect(() => {
    if (!shouldVirtualize || !isOpen || !scrollViewportRef.current) return;
    scrollViewportRef.current.scrollTop = 0;
    setVirtualScrollTop(0);
  }, [debouncedQuery, isOpen, shouldVirtualize]);
  ...
  React.useEffect(() => {
    if (activeIndex < 0 || interactionType.current !== "keyboard") return;

    if (shouldVirtualize && scrollViewportRef.current) {
      const viewport = scrollViewportRef.current;
      const itemTop = activeIndex * virtualItemHeight;
  ...
  const virtualEnd = shouldVirtualize
    ? Math.min(itemsToRender.length, Math.ceil((virtualScrollTop + maxHeight) / virtualItemHeight) + virtualOverscan)
    : itemsToRender.length;
  const visibleItems = shouldVirtualize
    ? itemsToRender.slice(virtualStart, virtualEnd).map((item, offset) => ({ item, index: virtualStart + offset }))
    : itemsToRender.map((item, index) => ({ item, index }));
              ...
              <div className="flex flex-col w-full">
                ...
                >
                  <ul
                    ...
                    style={shouldVirtualize ? { height: `${itemsToRender.length * virtualItemHeight + 12}px`, position: "relative" } : undefined}
                  >
                    {loading ? (
                          ...
                          return (
                            <M3AutocompleteItem
                              ...
                              className={cn(
                                ...
                                shouldVirtualize && "absolute inset-x-0"
                              )}
                              style={shouldVirtualize ? { top: `${index * virtualItemHeight + 4}px` } : undefined}
                            >
                              {isSelected && (
